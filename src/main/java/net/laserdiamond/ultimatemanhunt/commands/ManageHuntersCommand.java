            ...
            case REMOVE -> // Removing hunters
            {
                for (ServerPlayer serverPlayer : players)
                {
                    serverPlayer.getCapability(PlayerHunterCapability.PLAYER_HUNTER).ifPresent(playerHunter ->
                    {
                        ...
                        UMPackets.sendToPlayer(new HunterChangeS2CPacket(playerHunter), serverPlayer);
                        UMPackets.sendToAllTrackingEntityAndSelf(new HunterCapabilitySyncS2CPacket(serverPlayer.getId(), playerHunter.toNBT()), serverPlayer);
                        logHunterChange(commandContext.getSource(), serverPlayer, modifier, isBuffed);
                        i.getAndIncrement();
                    });
                }
