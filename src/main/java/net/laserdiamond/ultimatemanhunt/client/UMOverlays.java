...
public class UMOverlays implements LayeredDraw.Layer {
    ...
    {
        event.enqueueWork(UMOverlays::new);
    }
    ...
    private static class SpeedRunnerHunterDetectionOverlay implements LayeredDraw.Layer
    {
        ...
        public void render(GuiGraphics pGuiGraphics, DeltaTracker pDeltaTracker)
        {
            ...
            if (!isHunter) // Is the player a hunter?
            {
                if (lives > 0) // Does the speed runner have lives remaining?
                {
                    if ((distanceFromHunter < UMGame.HUNTER_DETECTION_RANGE) && (distanceFromHunter != 0) && !isOnGracePeriod) // Player has to be in hunter detection range, distance cannot be 0, and must not be on grace period
                    {
