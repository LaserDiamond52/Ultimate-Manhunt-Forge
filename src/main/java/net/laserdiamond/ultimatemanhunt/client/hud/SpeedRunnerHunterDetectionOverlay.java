...
public final class SpeedRunnerHunterDetectionOverlay implements UMHUDOverlay {
    ...
    public void onRender(LocalPlayer player, UMPlayer umPlayer, GuiGraphics guiGraphics, DeltaTracker deltaTracker)
    {
        ...
        {
            if ((distanceFromHunter < UMGame.HUNTER_DETECTION_RANGE) && (distanceFromHunter != 0) && !umPlayer.isSpeedRunnerOnGracePeriod()) // Player has to be in hunter detection range, distance cannot be 0, and must not be on grace period
            {
